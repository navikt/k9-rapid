...
data class Melding internal constructor(private val rawJson: String) {
    ...
    private val behovssekvensId = json.get("@behovssekvensId").asText().let { behovssekvensId ->
            try { ULID.parseULID(behovssekvensId) }
    ...
    fun harLøsningPå(behov: String): Boolean {
        if (behovssekvensId == null) return false
    ...
    fun <Løsning> løsningPå(løsningResolver: LøsningResolver<Løsning>): Pair<String, Løsning> =
        when (harLøsningPå(løsningResolver.behov())) {
            true -> Pair(behovssekvensId.toString(), løsningResolver.resolve(løsninger.get(løsningResolver.behov())))
            false -> throw IllegalStateException("Meldingen inneholder ikke løsning på ${løsningResolver.behov()}")
        }

    fun <Løsning> løsningPåOrNull(løsningResolver: LøsningResolver<Løsning>): Pair<String, Løsning>? =
        when (harLøsningPå(løsningResolver.behov())) {
